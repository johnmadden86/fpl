...
const fpl = {
    ...
    getGameDetails() {
        ...
        request(requestOptions, async (error, response, body) => {
            await body.elements.forEach((element, index, elements) => {
                footballers = elements;
            });
            logger.info('Got ' + footballers.length + ' footballers' + timeToLoad());
            footballers.forEach((footballer) => {
                requestOptions.url = fplUrl + 'element-summary/' + footballer.id;
                request(requestOptions, async (error, response, body) => {
                ...
                // executes immediately after bootstrap-static (598 times)
            });
            setTimeout(function () {
                footballers.forEach((footballer, index, array) => {
                    footballer.rating =
            ...
            // executes after above, but before 2nd async calls
        });
        // runs straight through, code executes before 1st async function (bootstrap-static)
    },
